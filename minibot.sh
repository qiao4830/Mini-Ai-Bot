...
# ==========================================
# 项目名称: Mini-AI-Bot V4.1 (准生产级旗舰版)
# 特色: 全链路流式回复 / 自动化日志滚动 / 环境安全隔离
...
# 🛡️ 日志滚动配置：最大 1MB，保留 1 个备份，防止撑爆小鸡硬盘
...
# 🚀 OpenAI 兼容接口流式解析逻辑 (DeepSeek/GPT)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if user_text.startswith('/ds ') and DS_KEY:
            ...
            await placeholder.edit_text(f"✨ [Gemini 2.5]\n\n{full_res}")
        else:
